...
// ValidateQuorum validates the quorum votes and if success returns the aligned votes back
func ValidateQuorum(ctx context.Context, sigVerifierFunc utils.SigVerifierFunc, quorumVotes *pb.QuorumVotes, computors *pb.Computors) (*pb.QuorumVotes, error) {
	if len(quorumVotes.QuorumVotesPerComputor) < types.MinimumQuorumVotes {
		return nil, errors.New("not enough quorum votes")
	}

	log.Printf("Proceed to filter aligned votes: %d\n", len(quorumVotes.QuorumVotesPerComputor))
...
func getAlignedVotes(quorumVotes *pb.QuorumVotes) (types.QuorumVotes, error) {
	votesHeatMap := make(map[[32]byte]types.QuorumVotes)
	for _, qv := range quorumVotes.QuorumVotesPerComputor {
		v := vote{
			Epoch:                         uint16(quorumVotes.QuorumTickStructure.Epoch),
			Tick:                          quorumVotes.QuorumTickStructure.TickNumber,
