...
type QuorumVotesFetcher interface {
	GetQuorumVotes(ctx context.Context, tickNumber uint32) (types.QuorumVotes, error)
}

func GetQuorumVotes(ctx context.Context, fetcher QuorumVotesFetcher, tick uint32) (*protobuff.QuorumVotes, error) {
	qubicQv, err := fetcher.GetQuorumVotes(ctx, tick)
	if err != nil {
		return nil, errors.Wrap(err, "fetching quorum votes")
	}

	qv := quorumQubicToProto(qubicQv)

	return qv, nil
}

func quorumQubicToProto(votes types.QuorumVotes) *protobuff.QuorumVotes {
	firstQuorumTickData := votes[0]
	protoQuorumTickData := protobuff.QuorumVotes{
		QuorumTickStructure:    qubicTickStructureToProto(firstQuorumTickData),
		QuorumVotesPerComputor: make(map[uint32]*protobuff.QuorumDiff),
	}

	for _, quorumTickData := range votes {
		protoQuorumTickData.QuorumVotesPerComputor[uint32(quorumTickData.ComputorIndex)] = qubicDiffToProto(quorumTickData)
	}
