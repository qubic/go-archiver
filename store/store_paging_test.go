...
func TestPebbleStore_TransfersPaging_GivenAllFitsPage_ThenReturnAll(t *testing.T) {
	...
	got, _, err := store.GetTransferTransactionsPaged(ctx, idOne, 12, 12, Pageable{0, 2}, Sortable{}, Filterable{}) // only first tick in range
	...
	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 10, 20, Pageable{0, 2}, Sortable{}, Filterable{}) // both ticks available
	require.NoError(t, err)
	require.Len(t, got, 1)
	require.Len(t, got[0].Transactions, 2)
	diff = cmp.Diff([]*pb.TransferTransactionsPerTick{forTickOne}, got, cmpopts.IgnoreUnexported(pb.TransferTransactionsPerTick{}, pb.Transaction{}))
	...
	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 15, 15, Pageable{0, 3}, Sortable{}, Filterable{}) // only second tick in range
	require.NoError(t, err)
	require.Len(t, got, 1)
...
func TestPebbleStore_TransfersPaging_GivenPartOfTickFitsPage_ThenPage(t *testing.T) {
	...
	got, _, err := store.GetTransferTransactionsPaged(ctx, idOne, 15, 15, Pageable{0, 2}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 1)
	require.Len(t, got[0].Transactions, 2)
	...
	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 15, 15, Pageable{1, 1}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 1)
	...
	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 15, 15, Pageable{1, 2}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 1)
	require.Len(t, got[0].Transactions, 1)
	diff = cmp.Diff([]*pb.TransferTransactionsPerTick{
...
func TestPebbleStore_TransfersPaging_GivenPageOutOfRange_ThenEmpty(t *testing.T) {
	...
	got, _, err := store.GetTransferTransactionsPaged(ctx, idOne, 15, 15, Pageable{1, 3}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 0) // all items in page 0

	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 15, 15, Pageable{3, 1}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 0) // all items in page 0
}
...
func TestPebbleStore_TransfersPaging_GivenPageSpansAllData_ThenReturnAll(t *testing.T) {
	...
	got, _, err := store.GetTransferTransactionsPaged(ctx, idOne, 10, 20, Pageable{0, 5}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 2)
	...
	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 10, 20, Pageable{0, 5000000}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 2)
	diff = cmp.Diff([]*pb.TransferTransactionsPerTick{forTickOne, forTickTwo}, got, cmpopts.IgnoreUnexported(pb.TransferTransactionsPerTick{}, pb.Transaction{}))
...
func TestPebbleStore_TransfersPaging_GivenPageSpansMultipleTicks_ThenCombineInPage(t *testing.T) {
	...
	got, _, err := store.GetTransferTransactionsPaged(ctx, idOne, 10, 20, Pageable{0, 3}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 2)
	require.Len(t, got[0].Transactions, 2)
	...
	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 10, 20, Pageable{1, 2}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 1)
	require.Len(t, got[0].Transactions, 2)
	diff = cmp.Diff([]*pb.TransferTransactionsPerTick{
	...
	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 10, 20, Pageable{2, 2}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 1)
	require.Len(t, got[0].Transactions, 1)
	diff = cmp.Diff([]*pb.TransferTransactionsPerTick{
	...
	got, _, err = store.GetTransferTransactionsPaged(ctx, idOne, 10, 20, Pageable{1, 3}, Sortable{}, Filterable{})
	require.NoError(t, err)
	require.Len(t, got, 1)
	require.Len(t, got[0].Transactions, 2)
	diff = cmp.Diff([]*pb.TransferTransactionsPerTick{
